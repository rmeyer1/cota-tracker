...
export default function BusMap({
  ...
  useEffect(() => {
    ...
    trafficCameras.forEach((camera) => {
      ...
      const popupContent = `
        <div style="font-family:var(--font-sans);font-size:13px;max-width:260px;">
          <div style="font-weight:700;font-size:13px;margin-bottom:4px;">${camera.location || "Traffic Camera"}</div>
          ${imgUrl ? `<img src="${imgUrl}" alt="Traffic camera" style="width:100%;border-radius:6px;margin-bottom:6px;" loading="lazy" />` : ""}
          ${view?.mainRoute ? `<div style="color:#666;font-size:12px;">Route: ${view.mainRoute}</div>` : ""}
          ${view?.direction ? `<div style="color:#666;font-size:12px;">Direction: ${view.direction}</div>` : ""}
          <div style="color:#888;font-size:11px;margin-top:4px;">${camera.description || ""}</div>
          ${view?.largeUrl ? `<a href="${view.largeUrl}" target="_blank" rel="noopener noreferrer" style="display:inline-block;margin-top:6px;font-size:12px;color:#3b82f6;">View Live Feed →</a>` : ""}
        </div>
      `;

      const icon = L.divIcon({
        className: "",
        html: `<div class="traffic-camera-marker" title="${camera.location?.replace(/"/g, "&quot;") || "Camera"}">
